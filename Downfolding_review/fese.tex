...
Diffusion Monte Carlo (DMC) has been shown to be a highly accurate method on transition metal oxide materials, where it has been shown to improve the description of the ground state properties and energy gaps~\cite{Foyevtsova2014, Wagner_Abbamonte, Zheng2015, Wagner2016}. % for the types of orbital excitations used to sample the Hilbert space.
...
We consider a multiband Hubbard model including one iron $s$ (\HHZ{4s?}), five iron $d$ states (\HHZ{3d?}), and three selenium $p$ states (\HHZ{3p or 4p?}):
\begin{align}
  H 
  &=
...
$E_0$ is an overall energy shift compared to the \textit{ab initio} hamiltonian.\HHZ{this sentence not very clear, maybe just say it is an overall energy shift without say "compared to"...}.
This set of parameters was chosen based on the matching pursuit method described in Sec.~\ref{sec:theory}.
We considered the set of 21 symmetry allowed $\epsilon$, $t$, $U$, and $J$ parameters.  \HHZ{I guess you have a set of parameters to consider, and then based on the matching pursuit, a subset stands out. So is 21 the total number of the big set?}
For this demonstration, we add parameters one at a time until the RMS error drops by less than 0.05 eV. 
With this criteria, \HHZ{maybe say XX parameters with most importance are selected?}  the final RMS error is 0.61 eV, the final coefficient of determination ($R^2$) is 0.84, and the next parameter in the algorithm reduces the $R^2$ by less than 0.01. 
...
All previous parameters may change at each step because the entire model is refitted with \HHZ{with$\to$ at?} each iteration.
The parameters are smoothly varying with the inclusion of new parameters, and their values are physically reasonable.
%generally take reasonable values.
