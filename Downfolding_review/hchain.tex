...
spin-restricted DFT-PBE calculations. The localized orbital basis upon which the density matrices (descriptors) are evaluated is obtained by generating intrinsic atomic orbitals (IAO) from the Kohn-Sham orbitals \HHZ{Maybe Kiel could clarify, the Kohn-Sham orbitals used for constructing IAO are those from spin unpolarized calculation?} orthogonizing using 
the L\"owdin procedure. The resulting orbitals are shown in Fig.~xx \HHZ{Could you plot these orbitals and replace the Wannier orbitals stuff?}
...
We compute the energies (expectation values of the Hamiltonian) and reduce reduce density matrices (descriptors) on all the wave functions. 
