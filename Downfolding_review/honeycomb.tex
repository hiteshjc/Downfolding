...
By constructing the one-body space from localizing orbitals from DFT calculations, we verify that 
the low energy degrees of freedom correspond to the $\pi$ orbitals in graphene and its $\pi$-only system
and $s$ orbitals in hydrogen, as shown in Fig.~\ref{fig:honeycomb_wan}. 
\HJC{Remove the next line because we repeat it 2 lines later?} We consider a one-band Hubbard model. 
