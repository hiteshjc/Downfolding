...
At the same time, more complex effective models can be commensurately more difficult to solve,  so one would like to also find an accurate effective model that is tractable \HHZ{``tractable" is a little bit vague. What need to be tracted?}. 
...
To address the need for a link between {\it ab initio} electron-level models and larger scale lattice models, downfolding has most commonly been carried out using approaches based on density functional theory (DFT). 
