...
The effective Hamiltonian dictates the behavior of the system on a coarse-grained level, where 'sub-grid' effects are folded into the parameters and form of the effective Hamiltonian. Fundamental concepts in condensed matter physics can be viewed as statements about the behavior of the effective Hamiltonian as the length scales change. \HHZ{just certain fundamental concepts, not all of them, right?}
For example, in a scale invariant system the effective Hamiltonian does not change as the length scale changes \HJC{eff Ham  changes but its "form" doesnt change? Would that be more technically correct?}, \HHZ{@Hitesh, The parameters in the Hamiltonian do not change at the critical point, the meaning of the ``orbital" does change (e.g., blocking spin in Ising model)},  while a system with emergent physics has a different form of effective Hamiltonian when described at different length scales\lucas{I'm nervous about saying these things; please take a look and comment.}. \HHZ{To me, the term "emergent" physics is more or less like the quasi-particle concept, or collective behavior of the system (if no quasi-particle exists) distinct from the microscopic degree of freedom / building block of the original Hamiltonian. So the comparison is more between the effective Hamiltonian with the original Hamiltonian. However, putting this together with the scaling invariant system, this statement might make people think that as we change scales (several), we have several different effective Hamiltonian describing different emergent physics. On the other hand, even the downfolded Hamiltonian has emergent physics other than the "effective" degree of freedom in the effective Hamiltonian (e.g. superconductivity in Hubbard model). }
...
The dominant effective model for quantum particles in condensed systems is band structure \HHZ{band structure is not an effective model in the context of above discussion but a result of the model}, and for metals, Fermi liquid theory \HHZ{Fermi liquid theory seems also more than an effective model..  How about: The dominant paradigm in condensed matter physics is band structure, and for metals, Fermi liquid theory, with an assumption of a mean field single-body effective Hamiltonian or a weakly correlated interacting Hamiltonian. However this paradigm}. 
...
Na\"ively, one might think to reconcile the fitting approach used in classical force fields with quantum models by matching eigenstates between the (now quantum) model and {\it ab initio} systems, varying the model parameters until the eigenstates match. \HHZ{Probably include citation to Lucas's previous work}
...
To resolve this, we develop a general theory for generating effective quantum models that is exact when the wave functions are sampled from the manifold of low-energy states, and is a good approximation when the sampled manifold is parallel to that manifold in the model space. \HHZ{The word "parallel" is not very clear. }
