...
In this paper, we have explained the AIDMD method that we have been developing, primarily in conjunction with the 
\emph{ab-initio} QMC approach. The practical motivation is to take data from first principles calculations 
as inputs for methods devised for lattice models, which can be simulated accurately at larger length scales. 
Since one is mapping a many body problem to a few body one, the relevant quantities of interest are only the reduced density matrices 
associated with the wavefunctions.  The density matrix based approach is appealing as it restores the 
democracy in the determination of the hopping and interaction parts of the effective Hamiltonian, and provides several 
...
Our experience suggests excitation spectra can be accurately determined to 0.2 eV (or less)~\cite{Changlani2015} 
and systematically improved with more data and more refined effective Hamiltonians.  In our view the main advantage is that 
the effective Hamiltonian opens up many avenues for physical quantities not easily calculated in ground state approaches. 
On a conceptual level, AIDMD reveals the types of terms are important and clues about what is missing. 
...
is still a method in its development stages, with room for several algorithmic improvements and new applications. In 
terms of algorithmic and conceptual developments we believe there are three main issues that need further research, 
\begin{itemize} 
	...
	which despite not being eigenstates, reveal the nature of the effective Hamiltonian. Automating its construction remains somewhat challenging. We propose Slater Jastrow wavefunctions and deforming the orbitals entering the Slater determinant as a way of probing the low energy manifold.
	\item Optimal choice of active orbitals (one body space):
	In the present work, we chose . Some ways of ..
	\item Form of the low energy model Hamiltonian:
	Many works in the literature have been to ~\cite{Georges}
	\item Other (non QMC) wavefunction based electronic structure methods:
	
\end{itemize} 

In terms of applications, we hope the method to be eventually applied to solids involving transition 
metals and/or rare earth elements, which are in the strongly correlated regime due to the presence of localized orbitals. 
Some challenging areas include the following,
\begin{itemize} 
	\item Magnetism: 
	The magnetism associated with strongly correlated Mott insulators is highly non trivial 
	and has become an active area of research in its own right. This is compounded by 
	the presence of geometric frustration, impurities and defects and 
	spin-orbital effects. For example, the frustrated nature of the two dimensional kagome lattice planes in 
	herbertsmithite harbors an exotic topological phase of matter - a "quantum spin liquid". While our understanding is primarily 
	from lattice model calculations of a Heisenberg model~\cite{Yan_Huse_White, Hermele_Wen, Iqbal_Becca, Changlani_motherkagome}, 
	contentious issues plague a complete understanding and connection to experiment; 
	there has been recent progress in downfolding herbertsmithite~\cite{Jeschke}. 
