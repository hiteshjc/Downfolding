...
(Note that the choice of spatial coordinates corresponds 
to the copper oxide plane to be the X-Y plane). The corresponding oxygen orbitals that hybridize most strongly 
with this orbital are the $p_x$ or $p_y$ orbitals. Thus, the minimal model for the cuprates involving the oxygens 
...
may itself be contingent on downfolding from a full-fledged \emph{ab-initio calculation}, 
of the type carried out in Refs.~\cite{Wagner_Abbamonte}. 
However, we steer clear of this debate here. Instead we use the 3-band model at half filling, 
as a prototypical example of what it \emph{means} to downfold to a simpler model - the one band Hubbard model, 
\begin{eqnarray}
...
be determined for given 3-band parameters, and 
$\tilde{d}$ are the effective \emph{d-like} orbitals. The latter are 
a mixture of copper and oxygen orbitals and this optimal transformation also remains an unknown. Thus, 
the determination of effective Hamiltonians is a \emph{dual} problem - what are the composite objects that give a 
...
models. We consider the case where only $\epsilon_p$,$t_{pd}$ and $U_{d}$ are non zero and 
...
Note that the transformation has been encoded to be as a linear one. However, 
...
that involve ring exchanges involving as many as 12 particles, certainly our simple minded 
transformation does not capture the search for such multi-body 
...
We connect The one particle basis in the transformed basis 
is related 
...
things,
1. The effective 
At half filling, the rotated d orbitals are exactly half filled
