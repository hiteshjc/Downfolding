...
democracy in the determination of the hopping and interaction parts of the effective Hamiltonian, and provides 
several sanity checks on the kind of accuracy one should expect. We have discussed three representative examples 
to discuss the conceptual and algorithmic aspects of AIDMD. 
...
fundamental interest. This, of course, is contingent on the cancellation of the large energy associated with the core electrons, 
whose role is primarily in renormalizing the effective interactions of the active (valence) electrons.
Our experience suggests spectra can be accurately determined to 0.2 eV (or less)~\cite{Changlani2015} 
and systematically improved with more data and more refined effective Hamiltonians.  In our view the main advantage is that 
the effective Hamiltonian opens up many avenues for quantities not easily calculated in ground state approaches - namely excitation spectra 
and finite temperature properties. On a conceptual level, AIDMD reveals the types of terms are important (and how much), 
what is missing and how it could be rectified. It is also possibly best suited for extended systems (solids) where 
there is often a clear separation of energy scales associated with the 
core, active (valence) and virtual electronic spaces. Finally we would like to emphasize that AIDMD, though conceptually simple, 
is still a method in its development stages, with room for several algorithmic improvements and new applications.

In terms of algorithmic and conceptual developments we believe there are three main issues that need further research and development, 
\begin{itemize} 
	\item Automated construction of database of low energy wavefunctions
	\item Optimal choice of active orbitals (one body space)
	\item Form of the low energy model Hamiltonian and number of free parameters
\end{itemize} 

We expect the method to be eventually applied to solids involving transition metals and/or rare earth elements, 
which are in the strongly correlated regime due to the presence of localized orbitals. These magnetism associated with the   
\begin{itemize} 
	\item Automated construction of database of low energy wavefunctions
	\item Choice of active orbitals
	\item Form of the low energy model Hamiltonian and number of free parameters
\end{itemize} 
