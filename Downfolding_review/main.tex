...
interactions are comparable to or more important than the kinetic energy. This has motivated an approach 
beyond the traditional band theory approach, where model Hamiltonians 
such as the Hubbard~\cite{Hubbard}, t-J~\cite{tJSpalek} and Heisenberg models involving only the valence electrons have been 
extensively studied analytically and numerically. 
Despite their great success in promoting our understanding of strongly correlated physics, their effectiveness 
...
the need to determine the effective Hamiltonians that can capture all the necessary details, while 
remaining simple enough to be simulated reliably.  
...
solution of the Schroedinger equation involving all the electrons. This reduction 
of the Hilbert space is known as ``downfolding". Many techniques are particularly suited for 
small local Hilbert spaces, these include, the density matrix renormalization group (DMRG), tensor networks, 
...
have traditionally been difficult in \emph{ab-initio} approaches. 
...
\HJC{Lucas handling this section......}
