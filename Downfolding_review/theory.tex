...
\item In the case of sampling an approximate LE, the error comes from non-parallelity of $E[\Psi]$ with the correct low energy manifold, up to a constant offset.
\item While $H_eff$ is unique, it has many potential representations and approximations. 
\item Our method can be applied to any manifold spanned by eigenstates
\item Model fitting is finding a compact approximation to $H_eff[\Psi]$. This is a high-dimensional space, so we use descriptors to do this.	
