...
\BDB{Did you intend the E in the $\mathcal{L}E$ to be nonscript?}
\begin{definition}
...
\BDB{Specify that $\oplus$ means direct sum? Maybe define what that means too? I'm not sure everyone will be familiar with this notation}
If $\ket{\Psi}\in {\mathcal LE}$ and $\ket{\Phi}\in {\mathcal H} \setminus {\mathcal LE}$, then $\ket{\Psi} \oplus \ket{\Phi} \in {\mathcal H}$.
In general in the following, we will use $\oplus 0$ to translate between the larger ${\mathcal H}$ and the smaller ${\mathcal LE}$. 
...
Assume $\ket{\Psi } \in {\mathcal LE}$ and $\frac{\delta  E[\Psi\oplus 0] }{\delta (\Psi\oplus 0)^*}  = \frac{\delta E_{eff}[\Psi]}{\delta \Psi^*} \oplus 0 $.
Then $(H_{eff}+C)|\Psi\rangle\oplus 0 = H (|\Psi\rangle \oplus 0)$, where $C$ is a constant.
\end{theorem}
\begin{proof}
Note that
\begin{align}
...
\begin{align}
	...
	\label{eqn:psieffder}
\end{align}
Since the derivatives are equal, $E[\Psi\oplus 0] - E_{eff}[\Psi]=C$ for some constant $C$.
So, setting Eqn~\ref{eqn:psider} equal to Eqn~\ref{eqn:psieffder},
\begin{align}
...
Theorem~\ref{theorem:matching} combined with Lemma~\ref{lemma:zeroderiv} mean that the eigenstates of $H_{eff}$ are be the same as the eigenstates of $H$ if its derivatives match $H$. 
Such $H_{eff}$ always exists. 
Let $H_{eff} = \sum_{n} e_n |n\rangle \langle n|$ where $|n\rangle$'s are eigenstates belong to ${\mathcal LE(H,N)}$. This satisfies $E[\Psi] = E_{eff}[\Psi]$ and $H_{eff}|\Psi\rangle = H |\Psi \rangle$ for any $\Psi$ in ${\mathcal LE(H,N)}$.  
...
\item In the case of sampling an approximate LE\BDB{Typo?}, the error comes from non-parallelity of $E[\Psi]$ with the correct low energy manifold, up to a constant offset.
...
\item Model fitting is finding a compact approximation to $H_{eff}[\Psi]$\BDB{you mean $E_{eff}[\Psi]$ or $H_{eff}$?}. This is a high-dimensional space, so we use descriptors to do this.	
...
Ideally\BDB{need comma?} one would be able to sample the entire low-energy space. 
...
\end{equation}
\BDB{Hat notation not used above}
...
\BDB{What many possiblities? Referring to the two possibilities in the previous sentence?}
In either case, when the model is accurate, the fits will be accurate.
If they \BDB{what are they?} are chosen incorrectly then intruder states can appear upon solution of the effective model. 
...
\BDB{Suggestion: combine previous two sentences as "If descriptor values available in the reduced Hilbert space are not represented in the sampled wave functions, then intruder states can appear in the solution of the effective model."}
In that case, the model fitting is an extrapolation instead of an interpolation, which has the usual problems \BDB{maybe should be more explicit?}.
...
\BDB{Suggest: "For this reason it is also desirable...". For some reason this language makes me think this point was discussed before, but actually, we said we don't need the eigenstates at all in the above.}
