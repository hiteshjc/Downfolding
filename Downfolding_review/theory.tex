...
Such a $H_{eff}$ always exists. 
Let $H_{eff} = \sum_{n=1}^N E_n |\Psi_n\rangle \langle \Psi_n|$ where $|\Psi_n\rangle$'s are eigenstates belong to $\mathcal{LE}(H,N)$. This satisfies $E[\Psi] = E_{eff}[\Psi]$ and $H_{eff}|\Psi\rangle = H |\Psi \rangle$ for any $|\Psi \rangle$ in $\mathcal{LE}(H,N)$.  
...
An effective way to accomplish this is through the use of descriptors, $d_j[\Psi]$, which map from ${\mathcal H} \rightarrow \mathbb{R}$.
Then we can approximate the energy functional as follows
\begin{equation}
E_{eff}[\Psi] \simeq \sum_i f_j(d_j[\Psi]),
\end{equation}
where $f_j$ are some parameterized functions.
...
\begin{tikzpicture}[scale=2,node distance = 3cm, auto]
    % Place nodes
    \node [block] (wfs) {Generate $\ket{\Psi_n} \in \mathcal{LE}$};
    \node [block, right of=wfs] (descriptors) {Generate $d_j[\Psi_n]$, $E[\Psi_n]$};
    \node [block, right of=descriptors] (assess) {Assess descriptors};
    \node [block, right of=assess] (ansatz) {Ansatz: $E_n \simeq \sum_j p_j d_{j}[\Psi_n]$};
...
At this point, one has collected the data $E_n$ and $d_{j}[\Psi_n]$. 
...
If descriptor values available in the reduced Hilbert space are not represented in the sampled wave functions, then intruder states can appear upon solution of the effective model. 
...
\label{eqn:EdP}
\end{equation}
where $ D_{n j} := d_j[\Psi_n]$ and $E_n := E[\Psi_n]$. If this can be done, the fitting problem is reduced to a linear regression optimization.
