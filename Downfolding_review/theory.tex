...
Practically, this can be implemented by first generating an \textit{ansatz} for the effective Hamiltonian in terms of operators. 
Then one generates wave functions $\ket{\Psi_i}$ in the low-energy subspace of the first principles Hilbert space ${\mathcal H}$.
Then one can compute $E[\Psi]$ using the expectation value of the first principles Hamiltonian and $E_eff[\Psi]$ by taking the expectation value of the operators in the ansatz for $H_{eff}$. 
It is not necessary to diagonalize either of the Hamiltonians; one must only be able to select wave functions from the low-energy space $\mathcal{LE}$.
