...
\end{equation}\HHZ{Should we change $\partial$ to $\delta$ since it is a functional derivative?}
...
We have thus reduced the problem of finding an effective Hamiltonian $H_{eff}$ that reproduces the low-energy spectrum of $H$ to matching the corresponding energy functionals $E[\Psi]$ and $E_{eff}[\Psi]$. \HHZ{We still need to find the form of $H_{eff}$ in order to compute $E_{eff}[\Psi]$, right?}
...
In our tests, we have successfully used LASSO\lucas{citation} and matching pursuit techniques\lucas{citation} to select high quality and compact model parameters. 
