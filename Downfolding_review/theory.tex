...
\begin{theorem}
If $E[\Psi] = E_{eff}[\Psi]$, for any $\Psi$ in $LE$, then $H_{eff}|\Psi\rangle = H |\Psi\rangle$ for any $\Psi$ in LE. 
\end{theorem}
This theorem also implies Theorem 2. $H_{eff}|\Psi\rangle = H |\Psi\rangle$ is saying that within LE, $H_{eff}$ behaves the same way as $H$. This gives the definition of effective Hamiltonian, and the theorem itself provides a way to find the $H_{eff}$. 

\begin{proof}
Since $E[\Psi] = E_{eff}[\Psi]$, we have $\frac{\delta E[\Psi]}{\delta \Psi^*}=\frac{\delta E_{eff}[\Psi]}{\delta \Psi^*}$. Note that
...
\end{proof}
Such $H_{eff}$ always exists. Let $H_{eff} = \sum_{n} e_n |n\rangle \langle n|$ where $|n\rangle$'s are eigenstates belong to $LE$. This satisfies $E[\Psi] = E_{eff}[\Psi]$ and $H_{eff}|\Psi\rangle = H |\Psi \rangle$ for any $\Psi$ in LE.  
