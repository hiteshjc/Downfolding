...
\begin{definition}
Let $\mathcal{LE}(H,N)$ be a subset of ${\mathcal H}$ spanned by $N$ vectors given by the lowest energy solutions to $H\ket{\Phi_i}=E_i{\Phi_i}$. \HHZ{Changing $\Phi_i$ to $\Psi_i$? Because, in the following part, $\Psi$ is used to denote vectors in the low energy manifold.} 
\end{definition}
...
Assume $\ket{\Psi } \in \mathcal{LE}$ and $\frac{\delta  E[\Psi\oplus 0] }{\delta (\Psi\oplus 0)^*}  = \frac{\delta E_{eff}[\Psi]}{\delta \Psi^*} \oplus 0 $. \HHZ{Is it better to  use $E[\Psi\oplus 0] - E_{eff}[\Psi]=C$  as condition of the theorem, since other parts are based on matching  $E[\Psi]$  and $E_{eff}[\Psi]$?}
...
So, setting Eq.~\eqref{eqn:psider} equal to Eq~\eqref{eqn:psieffder},
\begin{align}
