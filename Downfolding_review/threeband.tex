...
\item Section~\ref{subsection:1dhydrogen}: Hydrogen chain $\rightarrow$ one-band Hubbard model at half filling. Demonstrates basis sets for {\it ab initio} systems and the possibility to use this technique to determine the quality of a model to \HJC{for? instead of to?} a given physical situation.
...
We first determine the optimal operators $\tilde{d}_{i,\eta}$, which could be constructed by a unitary \HJC{It is not unitary, because we are truncating as well. I am blanking on the precise word} 
transformation of the oribital $d$ and $p$ orbitals, 
\begin{equation}
...
\HJC{We will now....} Now we will determine the transformation matrix ${\bf T}$. All RDMs in the three-band and one-band descriptions are also related via ${\bf T}$; 
...
The number of states $s$ was varied from three to six, depending on the energy window of interest. \HJC{For a selected low....} For a selecting low energy space, 
we optimize Eq.~\eqref{eq:C} first to determine the \HJC{optimal one-body operators...} one-body operators and then determine the parameters by linear regression fit [Eq.~\eqref{eq:linearfit_descriptor}].
...
Our results are summarized in Figure~\ref{fig:predictivity}. Panel (A) shows \HJC{remove "the" i.e. shows six...} the six representative parameter sets 
...
The three-band Hubbard model is on the $2\times 2$ cell with $U_d/t_{pd}=8$, $\Delta/t_{pd}=3$, and $t_{pd}=1.3$ \HJC{eV}.
}
\label{fig:windows} 
\end{figure}	
