...
are separated from the higher energy manifold. In the large $U$ limit correspond to the 4C2 states 
of the effective spin model in its $S_z=0$ sector, all of which correspond to \textit{d like} excitations, 
an aspect we will verify in this section. The eigenstates above this manifold involve \textit{p like} excitations which 
the 1-band model is not designed to capture. 
...
of the 3-band model for computing the cost in Eq.~\ref{eq:C}. The 
...
This matching procedure gives $(U/t)_1 \approx 14.1 $, $(U/t)_2 \approx 13.2 $, $(U/t)_3 \approx 12.7 $. Analogous estimates of $U/t$ were 
...
similar outcomes.) For our test example, this procedure provides an estimate of $U/t = 13.45 $ and $t = 0.3025 $ eV, 
the former in the correct range of the state-matching metric. (There is no error bar for this case since there 
are three unknown parameters including the constant shift and three entries in the DMD procedure). As was expected, and observed, when the 
1-band description is good, all estimates of $U/t$ essentially agree with each other. 
...
the lowest three eigenstates. (A) and (B) show the case of fixed $U_d/t_{pd}=8$ and varying $\Delta/t_{pd}$ 
...
Since DMD uses only information from the full (here, 3-band) model, it is imperative we 
perform other sanity checks on the accuracy of the downfolded model. An important check for the 1-band model is its 
ability to reproduce the low energy eigenenergies of the 3-band model. Fig.~\ref{fig:energyfit} shows 
the comparison of the energy gaps of the downfolded 1-band and full 3-band models. 
...
\caption{Comparison for energy gaps between the 3-band (black circles) and 1-band (red triangles) 
...
for optimizing the cost function in Eq.~\ref{eq:} and find similar values of $\alpha_i$ and hence ${\bf T}$. 
This confirms our expectation that the low energy space is describable by $\tilde{d_i}$ operators. 

However, as Fig.~\ref{fig:windows}(A) shows, based on how many eigenstates are used in the DMD 
one gets different estimates of $U/t$. This is because the DMD aims to provide the 1-band description 
that. If the model is not perfect within a given energy window, the DMD optimizes different cost functions. 
This is consistent with intuition from the renormalization group, which argues for the scale dependence 
of the effective parameters. This is further confirmed by Fig. . When all 6 states are used, 
...
(B) Comparison of energy gaps between the 3-band (black circles) and 1-band (red triangles) 
