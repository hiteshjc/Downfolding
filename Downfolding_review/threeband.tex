...
(with up to 32 carbons on a 2D honeycomb lattice). Since the 1-band Hubbard Hamiltonian occurs in most of our examples, 
we define it here,
\begin{eqnarray}
	\tilde{H} &=&  -t \;\sum_{\langle i,j \rangle} \tilde{d}_i^{\dagger} \tilde{d}_j + U \;\sum_{i} \tilde{n}^{i}_{\uparrow} \tilde{n}^{i}_{\downarrow}
\label{eq:oneband}
\end{eqnarray}
where $t$ and $U$ correspond 
...
The high $T_c$ superconducting cuprates~\cite{Bednorz1986} have parent Mott insulators that display 
rich phase diagrams on electron or hole doping~\cite{Dagotto_RevModPhys, LeeWen_RevModPhys}. 
