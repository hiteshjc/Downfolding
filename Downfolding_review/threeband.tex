...
to occupy the latter. When $\Delta/t_{pd}$ is fixed and $U_d/t_{pd}$ is increased, $U/t$ increases. 
As a way of avoiding the large $U_d$, the copper orbitals are forced to hybridize more with the oxygen ones; 
on the other hand the delocalization is suppressed in a bid to maintain only one electron per 
$\tilde{d}$. The net result of these effects is that the effective $t$ also increases, albeit only marginally; 
a trend also observed in $\alpha_1$.

An important check for the 1-band model is its ability to reproduce the low energy eigenstates 
of the 3-band model. Since the standard protocol uses only information from the 3-band model, it is imperative we 
perform other sanity checks on the accuracy of the downfolded model. 
Fig.~\ref{fig:energyfit} shows the comparison of the lowest six eigenstates of the 
...
Despite this good agreement, we caution that the reduced sensitivity of the energy to small RDM elements means 
