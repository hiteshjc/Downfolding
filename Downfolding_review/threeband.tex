...
\begin{eqnarray}
H &=&    \epsilon_p \sum_{j,\eta} n_{j,\eta} + \epsilon_{d} \sum_{i,\eta}  n_{i,\eta} 
	+ t_{pd} \sum_{\langle i,j \rangle, \eta} \text{sgn}(p_i,d_j) \Big( d_{i,\eta}^{\dagger} p_{j,\eta} + \text{h.c.} \Big) \nonumber \\
  & &   + U_p \sum_{j} n^{p}_{j,\uparrow} n^{p}_{j,\downarrow} + U_d \sum_{i} n_{i,\uparrow} n_{i,\downarrow} + V_{pd} \sum_{\langle i,j \rangle} n_j n_i\,,
\end{eqnarray}
...
\BDB{This was a run-on, I tried to correct it, but check that it's still what you wanted.}
...
\includegraphics[width=0.9\linewidth]{./Figures/three_band_figure.eps}
  \caption{\BDB{Not sure how important, but the light blue might not show up well in black and white/copied into a slide}Schematic for downfolding the three-band Hubbard model to the one-band Hubbard model. The oxygen orbitals are 
