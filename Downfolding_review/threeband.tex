...
Thus the model determination is a \textit{dual} problem \HHZ{dual usually means two are counterpart to each other in certain sense, if we know one, we know another as well. How about Thus the model determinations include the following two aspects} (1) what are the composite objects that give a 
...
All these issues will be clarified \HHZ{addressed?} in the remainder of the section. 
...
Higher body generalizations \HHZ{Higher body is not clear enough. Do you mean d=T c + .. c' c'' dagger c' + ... ?} of this relationship are also possible, but have not been considered here. 
...
The eigenstates outside of this manifold involve \textit{p-like} excitations which the one-band model is not designed \HHZ{designed $\to$ able?} to capture. 
...
satisfied to a relative error within 0.5 percent, verifying \HHZ{verifying $\to$ confirming?}  
the validity of Eq.~\eqref{eq:dc}. 
...
with qualitatively \HHZ{not quantatively?} similar results. 

The state-matching perspective \HHZ{perspective $\to$ approach?} was supported by DMD performed 
...
single particle energies of the copper and oxygen orbitals means the increased unfavorableness \HHZ{decreased favorableness?  or make it energetically unfavorable to hop between the two orbitals?} of holes 
...
the Hilbert space comprises of 112,911,876 basis states (\HHZ{compared to XXXX in the three-band model}). Roughly 200 Lanczos iterations were carried out, 
...
The lowest energy gap error is within $0.0004$ eV (1\% relative error).%, which in relative terms was a maximum error 
of 1 percent. The largest gap error in the third gap is of the order of $0.005$ eV (3\% relative error). %which in relative terms is about 3 percent. 
