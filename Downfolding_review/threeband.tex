...
are the effective interactions between them?
Moreover, the "best" effective Hamiltonian description depends on the energy scale of interest. 
Focusing on the lowest energy scales, we first use the exact knowledge of the lowest energy 
eigenstates of the 3-band and 1-band models and perform a state by state comparison in the low energy sector. 
Using this comparison gives us a set of 1-band Hubbard parameters which we use to test predictions on a larger cluster (8 unit cells). 
We then use the standard protocol (N-AIDMD) over a larger energy window and obtain the average parameters 
that describes the data in the entire window. These parameters are found to be different from 
the first set, and this indicates the importance of frequency dependent effects when downfolding.

First, we encode the relationship between the bare and effective operators as a linear transformation ${\bf T}$, 
\begin{equation}
