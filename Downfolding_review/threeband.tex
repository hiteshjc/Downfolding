...
%from \textit{exact} eigenstates themselves to downfold from a lattice model with more orbitals (three-band model) 
...
$\tilde{d}_{i,\sigma}$ is the effective one-particle operator associate with spatial orbital (or site) $i$ 
...
"best" describe the three-band data. The effective \textit{d-like} orbitals, $\tilde{d}$ that enter 
the low energy description are mixtures of copper and oxygen orbitals; this optimal transformation also remains an unknown. 
...
the ones that we focus on are those that are evaluated in eigenstate $s$, and are given by,
\begin{subequations}
\begin{eqnarray}
...
Fig.~\ref{fig:varyUdep} shows that in most regimes of the three-band model, the lowest six eigenstates 
...
the one-band model is not designed to capture. 
...
corresponding eigenstates of the one-band Hubbard model is directly available for arbitrary 
...
one-band description is good, all estimates of $U/t$ essentially agree with each other. 
...
%($\alpha_1$ through $\alpha_4$) and (Right) Effective one-band Hubbard $U/t$ and hopping $t$ (inset) 
...
and estimates of downfolded parameters of the one-band model for various parameter regimes of the three-band Hubbard model. 
