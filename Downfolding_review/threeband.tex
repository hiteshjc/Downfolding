...
for $U_d/t_{pd}=8$ and $\Delta/t_{pd}=3$. We find the optimal parameters of 
${\bf T}$ to be $\alpha_1=0.220$, $\alpha_2=0.044$, $\alpha_3=0.020$ and $\alpha_4=0.018$ 
and the corresponding trace conditions and orthogonality are individually 
satisfied to a relative error of the order of 0.1 percent.
...
%\end{eqnarray}
%\end{small}
Importantly, the off diagonal 1-RDM elements corresponding to nearest neighbors $\langle \tilde{d}_1^{\dagger} \tilde{d}_2 \rangle_s$; 
already provide estimates for the parameters of the effective model. 
...
we find $(U/t)_1 = $, $(U/t)_2$, $(U/t)_3 = $. Analogous estimates of $U/t$ could also be 
...
This perspective was confirmed by performing the standard protocol (N-AIDMD) with the same three low energy 
states using their energies and the computed values of  $\langle \tilde{d}_1^{\dagger} \tilde{d}_2 \rangle_s$ 
and $\langle \tilde{n}_{i,\uparrow} \tilde{n}_{i,\downarrow} \rangle_{s}$. 
...
As is expected, and observed, when the 1-band description is good, all estimates of $U/t$ 
...
We now explore trends in the 1-band description by monitoring the downfolded parameters as a function 
of varying $\Delta/t_{pd}$ and $U_d/t_{pd}$ in Fig.~\ref{fig:hamfitepdvary}. For example, 
when $U_d/t_{pd}=8$ is fixed and $\Delta/t_{pd}$ is increased, we find that both 
...
When $\Delta/t_{pd}$ is fixed and $U_d/t_{pd}$ is increased, $U/t$ increases also increases. 
As a way of avoiding the large $U_d$, the copper orbitals are forced 
to hybridize more with the oxygen ones, the net result is that the effective $t$ also increases, albeit marginally; 
a trend also observed in $\alpha_1$.
...
A primary objective of downfolding is to reduce the size of the effective Hilbert space; thus 
simulations of bigger unit cells can be carried out with the downfolded Hamiltonian. 
To show that this actually works well in practice for the 3-band case, 
