...
\renewcommand{\subfigimgone}[3][,]{%
  \setbox1=\hbox{\includegraphics[#1]{#3}}% Store image in box
  \leavevmode\rlap{\usebox1}% Print image
  \rlap{\hspace*{65pt}\vspace*{12pt}\raisebox{\dimexpr\ht1-5.3\baselineskip}{#2}}% Print label
  \phantom{\usebox1}
}
\renewcommand{\subfigimgtwo}[3][,]{%
  \setbox1=\hbox{\includegraphics[#1]{#3}}% Store image in box
...
\end{minipage}
\caption{(A) Effective parameters $t$ and $U/t$, for the one-band model obtained from downfolding the 4-copper ($2\times2$) unit cell 
...
(B and C) Comparison of the three lowest energy gaps for the 8-copper unit cell between the three-band and downfolded one-band 
models on varying $U_d/t_{pd}$ for (B) $\Delta/t_{pd}=3$ and (C) $\Delta/t_{pd}=5$ }
