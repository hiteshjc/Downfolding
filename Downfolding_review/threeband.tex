...
one-band (red triangles) Hubbard models on the $2\times2$ cell using the 
...
Panels (B) and (C) show the comparison of the three lowest energy gaps for the 8-copper unit cell between the three-band (black circles) 
and downfolded one-band (red triangles) models on varying $U_d/t_{pd}$ for (B) $\Delta/t_{pd}=3$ and (C) $\Delta/t_{pd}=5$ }
...
\end{tabular}
\caption{All results are for the $2\times2$ cell 
for $U_d/t_{pd}=8$ and $\Delta/t_{pd}=3$ and $t_{pd}=1.3$ eV. 
Panel (A) shows the downfolded values of $U/t$ and $t$ (inset) obtained by the DMD procedure using all eigenstates 
in a given energy window; increasing windows correspond to 3,5 and 6 low energy eigenstates. 
Panel (B) shows the comparison of energy gaps between the three-band (black circles) and one-band (red triangles) 
Hubbard models for different energy windows using the downfolded parameter values in that window.}
\label{fig:windows} 
\end{figure}	
